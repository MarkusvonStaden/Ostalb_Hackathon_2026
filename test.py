...
class AudioLoop:
    ...
    async def listen_audio(self):
        ...
        while True:
            ...
            cooldown = (time.monotonic() - self._playback_end_time) < 0.4
            if self.out_queue is not None and not self.is_playing and not cooldown:
    ...
    async def play_audio(self):
        ...
        while True:
            if self.audio_in_queue is not None:
                ...
                # Drain all immediately available chunks and write them in one call
                # to avoid gaps caused by per-chunk asyncio.to_thread overhead.
                extra = []
                while True:
                    try:
                        extra.append(self.audio_in_queue.get_nowait())
                    except asyncio.QueueEmpty:
                        break
                if extra:
                    bytestream = bytestream + b"".join(extra)
                self.is_playing = True
                await asyncio.to_thread(stream.write, bytestream)
                if self.audio_in_queue.empty():
                    self.is_playing = False
                    self._playback_end_time = time.monotonic()
                    # Drain stale mic data that accumulated during playback.
                    if self.out_queue is not None:
                        while True:
                            try:
